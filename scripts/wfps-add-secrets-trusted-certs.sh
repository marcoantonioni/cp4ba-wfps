...
usage () {
  log_msg "usage: $_me -c path-of-config-file -t path-of-trusted-certs-config-file -e full-path-to-target-environment-config-file"
  exit 1
}
...
getCertificate () {
  log_info "${_CLR_GREEN}Getting certificate from '${_CLR_YELLOW}$1${_CLR_GREEN}'${_CLR_NC}"
...
createSecret () {
  ...
  if [[ "${SECRET_ALREADY_SET}" == "1" ]]; then
    log_info "${_CLR_GREEN}Deleting old secret '${_CLR_YELLOW}$1${_CLR_GREEN}'${_CLR_NC}"
    oc delete secret $1 -n ${WFPS_NAMESPACE} 2>/dev/null 1>/dev/null
  fi
  log_info "${_CLR_GREEN}Creating new secret '${_CLR_YELLOW}$1${_CLR_GREEN}'${_CLR_NC}"
...
addSecrets () {
  ...
  do
    ...
    if [[ ! -z "${!_URL}" ]]; then
      log_info "${_CLR_GREEN}Working on '${_CLR_YELLOW}${!_URL}${_CLR_GREEN}' for secret '${_CLR_YELLOW}${!_CRT}${_CLR_GREEN}'${_CLR_NC}"
      getCertificate ${!_URL}
      ...
      if [[ $_OK -eq 0 ]]; then
        log_error "${_CLR_RED}Not a valid certificate from '${_CLR_YELLOW}${!_URL}${_CLR_RED}'${_CLR_NC}"
        log_error "${_CLR_RED}Secret '${_CLR_YELLOW}${!_CRT}${_CLR_RED}' not created/updated.${_CLR_NC}"
      fi
...
log_info "Using target environment config file '${_CLR_YELLOW}${TARGET_ENV_CONFIG_FILE}'${_CLR_NC}"
log_info "Using certs file '${_CLR_YELLOW}${TRUST_CERTS_FILE}'${_CLR_NC}"

sourceFile () {
  if [[ -f "${1}" ]]; then
    source ${1} 2> /dev/null 1> /dev/null
  else
    log_error "${_CLR_RED}ERROR File not found '${_CLR_YELLOW}${1}${_CLR_RED}'${_CLR_NC}"
    exit 1
  fi
}
